...
def scrape_betting_odds():
    ...
    try:
        ...
        # BFO uses table rows with fighters in pairs
        ...
        for table in tables:
            ...
            for i in range(0, len(rows) - 1, 2):
                try:
                    ...
                    # Get odds from each sportsbook cell
                    cells_a = row_a.select("td")
                    cells_b = row_b.select("td")

                    # First cell with odds data (usually DraftKings or FanDuel)
                    odds_a = ""
                    odds_b = ""
                    for cell in cells_a[1:]:
                        text = cell.get_text(strip=True)
                        if text and (text.startswith("+") or text.startswith("-")):
                            odds_a = text
                            break

                    for cell in cells_b[1:]:
                        text = cell.get_text(strip=True)
                        if text and (text.startswith("+") or text.startswith("-")):
                            odds_b = text
                            break
                    ...
                    # Calculate best odds (most favorable for each fighter)
                    best_a = max([int(o) for o in all_odds_a], default=0) if all_odds_a else 0
                    best_b = max([int(o) for o in all_odds_b], default=0) if all_odds_b else 0

                    if fighter_a and fighter_b and (odds_a or odds_b):
                        odds_data.append({
                            "fighter_a": fighter_a,
                            "fighter_b": fighter_b,
                            "odds_a": odds_a,
                            "odds_b": odds_b,
                            "best_odds_a": str(best_a) if best_a else "",
                            "best_odds_b": str(best_b) if best_b else "",
