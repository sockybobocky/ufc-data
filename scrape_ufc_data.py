...
def scrape_all_fighter_stats(fighter_records):
    ...
    for i, fr in enumerate(fighter_records):
        ...
        s = {"FIGHTER": name, "URL": url}
        for key, pat in [("SLPM",r"SLpM\s*:?\s*([\d.]+)"),("STR_ACC",r"Str\.?\s*Acc\.?\s*:?\s*([\d.]+)\s*%"),
                         ("SAPM",r"SApM\s*:?\s*([\d.]+)"),("STR_DEF",r"Str\.?\s*Def\.?\s*:?\s*([\d.]+)\s*%"),
                         ("TD_AVG",r"TD\s*Avg\.?\s*:?\s*([\d.]+)"),("TD_ACC",r"TD\s*Acc\.?\s*:?\s*([\d.]+)\s*%"),
                         ("TD_DEF",r"TD\s*Def\.?\s*:?\s*([\d.]+)\s*%"),("SUB_AVG",r"Sub\.?\s*Avg\.?\s*:?\s*([\d.]+)")]:
            m = re.search(pat, text)
            s[key] = m.group(1) if m else ""
        tott.append(s)
...
def scrape_upcoming_profiles():
    ...
    for i, fighter in enumerate(to_scrape):
        ...
        soup = BeautifulSoup(resp.text, "html.parser")
        bt = soup.get_text(" ", strip=True)
        s = {"fighter_name":fighter["name"],"fighter_url":fighter["url"]}
        for k, p in [("slpm",r"SLpM\s*:?\s*([\d.]+)"),("str_acc",r"Str\.?\s*Acc\.?\s*:?\s*([\d.]+)%?"),("sapm",r"SApM\s*:?\s*([\d.]+)"),("str_def",r"Str\.?\s*Def\.?\s*:?\s*([\d.]+)%?"),("td_avg",r"TD\s*Avg\.?\s*:?\s*([\d.]+)"),("td_acc",r"TD\s*Acc\.?\s*:?\s*([\d.]+)%?"),("td_def",r"TD\s*Def\.?\s*:?\s*([\d.]+)%?"),("sub_avg",r"Sub\.?\s*Avg\.?\s*:?\s*([\d.]+)")]:
            m = re.search(p, bt)
