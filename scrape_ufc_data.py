...
# ═══════════════════════════════════════════════════
# 3. BETTING ODDS from BestFightOdds
# ═══════════════════════════════════════════════════
def scrape_betting_odds():
    print("\n[3/5] Scraping betting odds from BestFightOdds...")
    odds_data = []

    try:
        resp = SESSION.get("https://www.bestfightodds.com/", timeout=15)
        resp.raise_for_status()
        soup = BeautifulSoup(resp.text, "html.parser")

        # BFO structure: event headers (h2/a) followed by tables with fighter rows
        # We want to find UFC events and get their odds
        current_event = ""
        is_ufc_event = False

        # Process all elements in order to track which event we're in
        for element in soup.select("table, [class*='table-header'], h2, a[href*='/events/']"):
            # Check for event title links
            tag_text = element.get_text(strip=True)
            href = element.get("href", "")

            if "/events/" in href or (element.name in ["h2", "h3"] and tag_text):
                current_event = tag_text
                is_ufc_event = "ufc" in tag_text.lower()
                continue

            if element.name != "table":
                continue

            # Process table rows in pairs (fighter A, fighter B)
            ...
            for i in range(0, len(rows) - 1, 2):
                try:
                    ...
                    link_a = row_a.select_one('a[href*="/fighters/"]')
                    link_b = row_b.select_one('a[href*="/fighters/"]')
                    if not link_a or not link_b:
                        continue

                    fighter_a = link_a.get_text(strip=True)
                    fighter_b = link_b.get_text(strip=True)
                    if not fighter_a or not fighter_b:
                        continue

                    # Extract all odds values from cells
                    all_odds_a = []
                    all_odds_b = []
                    for cell in row_a.select("td")[1:]:
                        text = cell.get_text(strip=True)
                        if text and re.match(r'^[+-]\d+', text):
                            all_odds_a.append(text)
                    for cell in row_b.select("td")[1:]:
                        text = cell.get_text(strip=True)
                        if text and re.match(r'^[+-]\d+', text):
                            all_odds_b.append(text)

                    odds_a = all_odds_a[0] if all_odds_a else ""
                    odds_b = all_odds_b[0] if all_odds_b else ""
                    ...
                    # Calculate best odds
                    try:
                        best_a = str(max(int(o) for o in all_odds_a)) if all_odds_a else ""
                    except ValueError:
                        best_a = ""
                    try:
                        best_b = str(max(int(o) for o in all_odds_b)) if all_odds_b else ""
                    except ValueError:
                        best_b = ""
