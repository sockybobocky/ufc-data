...
def save_weights(weights, test_acc, cv_acc, model=None, X=None):
    """Save weights and full model to JSON for the app to use."""
    # Save simplified weights (fallback)
    output = {
        'version': '2.0',
        'model': 'XGBoost',
    ...
    with open('ufc_ml_model.json', 'w') as f:
        json.dump(full_model, f)  # No indent to keep file size small

    model_size = os.path.getsize('ufc_ml_model.json')
    print(f"  Saved full model to: ufc_ml_model.json ({model_size:,} bytes, {len(trees)} trees)")
    print(f"  Test accuracy: {test_acc:.1%}")
...
def main():
    ...
    # Save
    save_weights(weights, test_acc, cv_acc, model, X)
